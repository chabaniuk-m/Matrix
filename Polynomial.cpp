...
std::vector<int> Polynomial::getRoots()
{
	std::vector<int> roots;
	Polynomial divided(*this);

	if (divided.getLeastDegree() > 0)                   //знаходимо всі нульові корені
	{
	...
	for (auto& x0 : divisors)								//перебираємо всі прості дільники вільного члена
	{
		if (divided.empty()) break;
		while (divided.value(x0) == 0)						//якщо значення полінома в точці == 0 знаходими його кратність
		{
			roots.push_back(x0);
			Polynomial pol(-x0);
	...
	return roots;						//повертаємо масив де кратні корені записуються попідряд
}
